 ...
 * @author balmermi
 */
/*package*/ class PopulationWriterHandlerImplV7 implements PopulationWriterHandler {
	@SuppressWarnings("unused")
	private static final Logger log = Logger.getLogger( PopulationWriterHandlerImplV7.class );

	// TODO: infrastructure to inject converters
	private final AttributesXmlWriterDelegate attributesWriter = new AttributesXmlWriterDelegate();
	private final CoordinateTransformation coordinateTransformation;

	PopulationWriterHandlerImplV7(CoordinateTransformation coordinateTransformation) {
		...
		for (Plan plan : person.getPlans()) {
			...
			for (PlanElement pe : plan.getPlanElements()) {
				if (pe instanceof Activity) {
					...
					if (route != null) {
						PopulationWriterHandlerImplV7.startRoute(route, out);
						PopulationWriterHandlerImplV7.endRoute(out);
					}
			...
			PopulationWriterHandlerImplV7.endPlan(out);
		}
		PopulationWriterHandlerImplV7.endPerson(out);
